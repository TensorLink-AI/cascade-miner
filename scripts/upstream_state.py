...
KEYS: tuple[tuple[str, str, str], ...] = (
    ...
    ("Submission limits", "generator.corpus_target_points", "points-denominated materialised-drain cap; 0 = legacy exact-count drain"),
    ("Submission limits", "generator.max_generate_seconds", "batch-drain budget (CPU rlimit + wall) for a materialised run"),
    ("Submission limits", "generator.stream_stall_seconds", "per-series stall window before a streaming generator is killed"),
    ...
    ("Submission limits", "generator.interface_version", "record-carrier version generate() may yield"),
    ("Submission limits", "generator.max_payload_bytes", "canonical payload byte cap; 0 = off"),
    ...
    ("Round mechanics", "round.finalists", "challengers promoted from heat to duel (legacy key)"),
    ("Round mechanics", "round.max_finalists", "cap on the tied cohort the heat advances; 1 = tie logic off"),
    ("Round mechanics", "round.tie_runoff_windows", "run-off eval size for a tied heat top; 0 = advance capped, no re-score"),
    ("Round mechanics", "round.init_gate_mode", "heat shadow row scoring the round's warm-start init"),
    ...
    ("Training contract", "training.warm_lr_scale", "LR multiplier for warm-started rounds"),
    ("Training contract", "training.ema_decay", "EMA finished form — the scored artifact when set"),
    ("Training contract", "training.gen_seed_mix", "derived generation seeds interleaved into the corpus"),
    ...
    ("Decision statistic", "eval.mix_from_block", "jittered round-mix activation block; 0 = legacy uniform draw"),
    ("Decision statistic", "eval.mix_target_windows", "jittered draw size — overrides the requested window count when active"),
    ("Decision statistic", "eval.mix_tier_from_block", "two-tier even-by-domain split activation block"),
    ...
    ("Decision statistic", "scoring.init_gate_mode", "duel-side init-baseline floor: off/shadow/enforce"),
    ("Decision statistic", "scoring.margin_mode", "margin denomination: level vs warm-start increment"),
